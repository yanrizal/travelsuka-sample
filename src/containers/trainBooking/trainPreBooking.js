...
class TrainPreBooking extends React.Component {
  ...
  componentDidMount() {
    ...
    if (twoway) {
      ...
      this.setState({
        hours: fs[0].duration.hours,
        minutes: fs[0].duration.minutes,
        hours2: fs2[0].duration.hours,
        minutes2: fs2[0].duration.minutes,
      })
        ...
        <div className="top-book">
          ...
          <div className="container">
            <div className="row">
              ...
              <h3 className="your-i" style={{marginBottom:'20px'}}>Train Detail</h3>
              ...
              <div className="row" style={{marginBottom:'20px'}} key={index}>
                <div className="col-md-2">
                  ...
                  {this.state.seat[flight.fares[0].cabin]}</span>
                </div>
                <div className="col-md-6">
                 <div>
                  ...
                  <div style={{margin:'20px 0 20px 20px'}}>
                    <span style={{fontSize:'14px'}}>&nbsp;{this.state.hours}h {this.state.minutes}m</span>
                  </div>
                  <div>
                    ...
                    <div className="row" style={{marginBottom:'20px'}} key={index}>
                      ...
                      <div className="col-md-6">
                        <div>
                          ...
                          <div style={{margin:'20px 0 20px 20px'}}>
                            <span style={{fontSize:'14px'}}>&nbsp;{this.state.hours2}h {this.state.minutes}m</span>
                          </div>
                          <div>
                          <span><span className="circle-dot2"></span>{moment(flight.sta).format("HH:mm")}</span>&nbsp;
                          <span>{flight.arrivalAirport.city} ({flight.arrivalAirport.code})</span><br/>
            ...
            <div className="row">
              <div className="col-md-12">
              <h3 className="your-i" style={{marginBottom:'20px'}}>Summary</h3>
              <div className="clear"></div>
              {!this.props.twoway &&
              <div className="p20" style={{paddingLeft:0}}>

              <p>Summary Pergi</p>
              <ul style={{listStyle:'none',paddingLeft:0}}>
                {/*{this.props.adultCount > 0 &&
                <li>Adult Basic Fare ({this.props.adultCount}x): Rp {formatRp(totalAdultFare - tax - vat)}</li>
                }
                {this.props.childCount > 0 &&
                <li>Child Basic Fare ({this.props.childCount}x): Rp {formatRp(totalChildFare - tax - vat)}</li>
                }
                {this.props.infantCount > 0 &&
                <li>Infant Basic Fare ({this.props.infantCount}x): Rp {formatRp(totalInfantFare - tax - vat)}</li>
                }
                <li>VAT: Rp {formatRp(vat)}</li>
                <li>Pax Service Charge (PSC): {tax}</li>*/}
                <li style={{fontWeight:'600',fontSize:'20px'}}>Total: Rp {formatRp(totalAdultFare + totalChildFare + totalInfantFare)}</li>
              </ul>
              </div>
              }
              {/*this.props.twoway &&
                ...
                <ul style={{listStyle:'none',paddingLeft:0}}>
                  {this.props.adultCount > 0 &&
                  <li>Adult Basic Fare ({this.props.adultCount}x): Rp {formatRp(totalAdultFare2 - tax2 - vat2)}</li>
                  }
                  {this.props.childCount > 0 &&
                  <li>Child Basic Fare ({this.props.childCount}x): Rp {formatRp(totalChildFare2 - tax2 - vat2)}</li>
                  }
                  {this.props.infantCount > 0 &&
                  <li>Infant Basic Fare ({this.props.infantCount}x): Rp {formatRp(totalInfantFare2 - tax2 - vat2)}</li>
                  }
                  <li>VAT: Rp {formatRp(vat2)}</li>
                  <li>Tax Service Charge (TSC): {tax2}</li>
                  <li style={{fontWeight:'600',fontSize:'20px'}}>Total: Rp {formatRp(totalAdultFare2 + totalChildFare2 + totalInfantFare2)}</li>
                </ul>
                </div>
              */}
              <br/>
              {this.props.twoway &&
                <p style={{fontWeight:'bold',fontSize:'24px'}}>Grand Total {formatRp(totalAdultFare + totalChildFare + totalInfantFare + totalAdultFare2 + totalChildFare2 + totalInfantFare2)}</p>
              }
              <Link to={`/kereta-api/booking/${this.props.match.params.id}`} className="btn btn-choose pull-left" style={{color:'#FFF'}}>Booking</Link>
            </div>
            </div>
          </div>
        </div>
