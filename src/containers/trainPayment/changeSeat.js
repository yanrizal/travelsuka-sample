...
class ChangeSeat extends React.Component {
  ...
  componentDidMount(){
    ...
    this.props.getSeatMap({
      ...
      const seat = result.data.seatMap[0]
      ...
      //const pD = this.state.originPassengerDetails.concat(bkSummary.passengerDetails);
      //console.log(pD,'pd')
      

      this.setState({
        passengerDetails: bkSummary.passengerDetails,
        //originPassengerDetails: Object.assign({}, pD),
        passenger: bkSummary.passengerDetails.length,
        seatData: seat,
        loading: false,
        wagonIdSelected: seat.Wagons[0].WagonId,
      },() => {
        bkSummary.passengerDetails.map((item,index) => {
  ...
  handleChooseSeat = (seatCode, seatNumber, available, e) => {
    //console.log('originPassengerDetails',`${this.state.selected}-${seatCode}/${seatNumber}`)
  ...
  handleChangeSeat = e => {
    const { bookingSummary, payloadSearch, selectedFlight } = this.props;
    let departureSeats = [];
    const bkSummary = bookingSummary.bookingsSummary[0];
      ...
      <div className="container">
        ...
        <div className="col-md-3">
          ...
          {this.state.passengerDetails.map((item, index) => {
            ...
            {/*<p style={{fontSize:'14px'}}>{index + 1}. {item.firstName}<span className="pull-right">{item.seatInfo[0].WagonId} {item.seatInfo[0].WagonNo}/{item.seatInfo[0].SeatNumber}{item.seatInfo[0].SeatCode}</span></p>*/}
          {!this.state.loading &&
